window.PassportModal = {
    async open() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to load passport modal: ${response.status} ${response.statusText}`);
            }
    ...
    showLoadingModal() {
        const loadingHtml = `
            <div id="passportModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" role="dialog" aria-labelledby="passportModalTitle" aria-hidden="true">
    ...
    replaceModalContent(html) {
        const existingModal = document.getElementById('passportModal');
        if (existingModal) {
    ...
    showErrorModal(message = 'Failed to load passport information. Please try again.') {
        const existingModal = document.getElementById('passportModal');
        if (existingModal) {
